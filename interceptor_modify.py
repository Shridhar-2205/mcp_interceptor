...
...but with a side effect: every intercepted ``tools/call`` is recorded and the
interceptor (re)writes a sibling Python file, ``replay_calls.py``, into a
runnable script that replays those exact calls against the server. So watching
the traffic actually rewrites another .py file on disk, live.
...
def _record_if_tool_call(raw: bytes) -> None:
    ...
    with _lock:
        ...
        _write_replay_file()
        sys.stderr.write(
            f"[modify] recorded tool call #{len(_calls)} ({name}) -> rewrote {os.path.basename(TARGET_FILE)}\n"
        )
        sys.stderr.flush()


def _write_replay_file() -> None:
    """Rewrite replay_calls.py as a runnable script for the calls seen so far."""
    lines = [
        "#!/usr/bin/env python3",
        '"""AUTO-GENERATED by interceptor_modify.py — do not edit by hand.',
        "",
        f"Recorded {len(_calls)} tool call(s) at {_ts()} by intercepting the",
        "client<->server MCP traffic. Run this file to replay them against the",
        "server directly:  python replay_calls.py",
        '"""',
        "",
        "import asyncio",
        "import os",
